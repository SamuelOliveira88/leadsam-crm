select cron.unschedule('distribuir-lote-diario-planilha');
...
begin
  ...
  for v_c in select * from (values (v_sofia, 20, 1), (v_sidney, 3, 7)) as t(cid, meta, cada) loop
...
drop function if exists public.distribuir_lote_diario_planilha();

select cron.schedule(
  'distribuir-gota-planilha',
  '0,30 12-21 * * *',
  $$select public.distribuir_gota_planilha();$$
);